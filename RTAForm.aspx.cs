...
namespace UC_LIVE
{
    public partial class RTAForm : System.Web.UI.Page
    {
                    ...
                    using (System.Data.IDataReader dataReader = sqlCommand.ExecuteReader())
                        ...
                        if (dataReader.Read())
                        {
                           // HireStartDate.InnerHtml = string.Concat("Claim Ref: ", ClaimID, "<br/><br/>", "Date:", Convert.ToDateTime(dataReader["dateupdated"]).ToString("dd-M-yyyy"));
                            solicitoraddress.InnerHtml = string.Concat((dataReader["Company"]), "</br>", Convert.ToString(dataReader["Address1"]), "<br/>", Convert.ToString(dataReader["Address2"]), "<br/>", Convert.ToString(dataReader["City"]), "<br/>", Convert.ToString(dataReader["Postcode"]), "<br/>");
                        }
                }
            }
